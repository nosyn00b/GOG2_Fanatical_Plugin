...
-------------------------------------------------------------------
This version was modified in 2021 by nosyn00b <nosyn00b71@gmail.com>
to fix base64 encoding/deconding to better fit with the 
JS-side LZ-based compression algorithm and to add UriSafe methods to 
LZstring version 1.4.4 (by Pieroxy <pieroxy@pieroxy.net>) 
"""
...
class LZString:
    ...
    def compressToBase64(self, string):
        ...
        while  (i < strlen*2-1 and (i < strlen*2-2 or odd==0)):
        #for i in range(0, (strlen * 2)-1, 3):
            if (i % 2) == 0:
            ...
            if (math.isnan(chr2) or (i==strlen*2 and odd)):
                enc3 = 64
                enc4 = 64
            elif (math.isnan(chr3) or (i==strlen*2-1 and odd)):
                enc4 = 64
    ...
    def compressToEncodedURIComponent(self, string):
        if string is None:
            return ''
        ...
        #for i in range(0, (strlen * 2)-1, 3):
            ...
            output += (
                self.safeKeyStr[enc1] +
                self.safeKeyStr[enc2] +
                self.safeKeyStr[enc3] +
                self.safeKeyStr[enc4]
            )
    ...
    def decompressFromBase64(self, iinput):
        ...
        #print('decompressFromBase64 iinput='+iinput)
        ...
        if (flush): 
            output = output + chr(output_)
            output = output + chr(0)
            output = output + chr(ord(output[0])|256) + output[1]
        else:
            output = output + chr(0)

        #print('base64 before decompress='+output)
        return self.decompress(output)

    def decompressFromEncodedURIComponent(self, iinput):
        if iinput is None:
            return ''
        ...
        iinput = re.sub(r'[^A-Za-z0-9\-_~]', '', iinput)

        #manages strings without trailing characters (generated by JS-side original algorithm)
        if (len(iinput)%4 != 0):
            rrange=4*(len(iinput)//4+1)
            iinput = (iinput+"~~~")[0:rrange]
        #print('decompressFromEncodedURIComponent iinput='+iinput)

        while i < len(iinput):
            enc1=self.safeKeyStr.index(iinput[i])
            i=i+1
            enc2=self.safeKeyStr.index(iinput[i])
            i=i+1
            enc3=self.safeKeyStr.index(iinput[i])
            i=i+1
            enc4=self.safeKeyStr.index(iinput[i])
            i=i+1
        ...
        #print('encodeduri before decompress='+output)
        return self.decompress(output)
